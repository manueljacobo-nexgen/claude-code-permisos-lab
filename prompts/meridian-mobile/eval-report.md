...
## Correccion post-hoc, ronda 1: bug real en el runner (`--tools ""`)

En primera revision se encontro que `run-eval.sh` usaba `--tools ""` para bloquear tools en la condicion "sin herramienta" (V0-V3). La ayuda del CLI documenta que `""` desactiva todas las tools, pero **no es cierto en la practica**: se probo directamente (pidiendole a un modelo de prueba que usara Bash con `--tools ""` activo) y el modelo igual ejecuto el comando. Se cambio a `--disallowedTools` con una lista explicita.

## Correccion post-hoc, ronda 2: el analisis de impacto de la ronda 1 tambien estaba mal

Una segunda revision (workflow de code review, effort alto) encontro que la conclusion de "sin impacto" de la ronda 1 se apoyaba en un razonamiento invalido: **`--output-format text` nunca muestra bloques de tool-use, haya pasado o no una invocacion real** — la ausencia de evidencia de tool-use en el texto no es evidencia de ausencia de tool-use. Ademas, la lista de `--disallowedTools` de la ronda 1 (`Bash,Edit,Write,Read,WebFetch,WebSearch`) resulto ser incompleta: al re-verificar, `ToolSearch` se colo sin estar bloqueado (detectado en una prueba deliberada con v4-prompt.md sin permitir Bash — el modelo igual invoco `ToolSearch` dos veces).

Se corrigio de raiz:
- `run-eval.sh` ahora corre con `--output-format stream-json` y verifica programaticamente que no aparezca ningun bloque `tool_use` cuando el run se supone sin tools — si aparece, el script falla con exit 1 y lista que tool se invoco, en vez de asumir silenciosamente que el flag funciono.
- La lista de `--disallowedTools` se amplio (`Bash,Edit,Write,Read,Glob,Grep,WebFetch,WebSearch,Task,NotebookEdit,TodoWrite,Skill,Workflow,ToolSearch,AskUserQuestion,Artifact,EnterPlanMode,ExitPlanMode,ScheduleWakeup,ReportFindings,mcp__*`) como mitigacion adicional (defensa en profundidad, no reemplaza la verificacion del trace).
- Se re-corrieron con el harness corregido y con verificacion real de trace: `edge-aritmetica` completo (V0 x3, V1-V3 x1, V4 x3, V5 x1 — el unico caso donde la disponibilidad de una tool es la variable causal de la narrativa) y un spot-check de `control`/`politica` (donde la tool nunca fue relevante para la narrativa). Resultado: **ningun run "sin tools" disparo el guard** — se confirmo con evidencia de trace, no por ausencia de texto, que el modelo nunca invoco una tool en esas condiciones. Los 10 reruns de `edge-aritmetica` dieron $10.00 (correcto), consistente con — pero no identico a — el muestreo original (que tenia 1 fallo de 3 en V0). Los archivos originales committeados (`results/v0-run1_edge-aritmetica.txt`, etc.) **no se sobreescribieron**: la re-verificacion se guardo aparte en `results/reverify-*_edge-aritmetica.txt` para no destruir la evidencia original de varianza ya documentada arriba.

**Nota de comportamiento (no afecta el pass/fail):** en `reverify-v4-run2`, pese a tener Bash permitido, el modelo respondio "No calculator tool available here" y de todas formas calculo bien a mano ($10.00) — el modelo no siempre invoca la tool aunque este disponible y se le pida, aunque en este caso el resultado siguio siendo correcto.

**Conclusion honesta:** la primera ronda de "correccion" tambien tenia un bug (an analisis de impacto invalido + una lista de bloqueo incompleta). Quedo resuelto recien en esta segunda ronda, verificando con el trace real en vez de con la ausencia de texto visible.
